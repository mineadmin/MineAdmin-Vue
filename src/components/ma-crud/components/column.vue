<template>
  <template v-for="row in props.columns" :key="row[props.options.pk]">
    <template v-if="! row.hide">
      ...
      <a-table-column :title="row.title" :data-index="row.dataIndex" :width="row.width" :ellipsis="true"
        :tooltip="row.dataIndex === '__operation' ? false : true" :align="row.align || 'left'" :fixed="row.fixed"
        :sortable="row.sortable" v-if="!row.hide">
            ...
            <a-space size="mini">
              ...
              <slot name="operationCell" v-bind="{ record, column, rowIndex }">
                ...
                <a-link v-if="
                  ...
                  && !props.isRecovery
                  && ($common.auth(props.options.edit.auth || [])
                    || (props.options.edit.role || []))
                " type="primary" @click="editAction(record)">
                  <icon-edit /> {{ props.options.edit.text || '编辑' }}
                </a-link>

                <a-popconfirm content="确定要恢复该数据吗?" position="bottom" @ok="recoveryAction(record)" v-if="
                  props.options.recovery.show
                  && props.isRecovery
                  && ($common.auth(props.options.recovery.auth || [])
                    || (props.options.recovery.role || []))
                ">
                  <a-link type="primary">
                    <icon-undo /> {{ props.options.recovery.text || '恢复' }}
                  </a-link>
                </a-popconfirm>

                <a-popconfirm content="确定要删除该数据吗?" position="bottom" @ok="deleteAction(record)" v-if="
                  props.options.delete.show
                  && ($common.auth(props.options.delete.auth || [])
                    || (props.options.delete.role || []))
                ">
                  <a-link type="primary">
                    <icon-delete /> {{ props.isRecovery ? props.options.delete.realText || '删除' : props.options.delete.text || '删除'
                    }}
                  </a-link>
                </a-popconfirm>
              </slot>
