...
<template>
  <div class="w-full">
    <a-spin :loading="loading" tip="加载中..." class="w-full">
      ...
      <a-form ref="crudForm" :model="form" label-align="right" @submit="submit">
...
const add = () => {
  ...
  props.columns.map(item => tmp[item.dataIndex] = '')
  form.datas.push(tmp)
}
...
const submit = (data) => {
  emit('submit', data.values.datas, done)
}
...
const handlerCascader = (val, column) => {
  if (column.cascaderItem && isArray(column.cascaderItem) && column.cascaderItem.length > 0 && val) {
    ...
    column.cascaderItem.map(async name => {
      const dict = props.columns.value.filter(col => col.dataIndex === name && col.dict )[0].dict
